...
#ifndef PARDELAUNAY_DELAUNAY_H
#define PARDELAUNAY_DELAUNAY_H

#endif //PARDELAUNAY_DELAUNAY_H
...
struct Delaunay {
    ...
    Delaunay(const Points& P) :
            ...
            n(P.size()) {
        points = P;
        auto M_x = parlay::max_element(points, [&](point& a, point& b) {return a.x < b.x;}) -> x;
        auto M_y = parlay::max_element(points, [&](point& a, point& b) {return a.y < b.y;}) -> y;
        auto M = std::max(M_x, M_y);
        // enclosing triangle
        point p0{n,0.0, 3.0 * M};
        point p1{n+1, 3.0 * M, 0.0};
        point p2{n+2, -3.0 * M, -3.0 * M};
...
//Delaunay delaunay(const Points& P) {
//    Delaunay dt(P);
//    return dt;
//}
