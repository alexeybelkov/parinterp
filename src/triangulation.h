...
//    Delaunator* delauany;
    ...
    parlay::sequence<double> barycentric_coordinates(double x, double y, uint32_t t);
    parlay::sequence<double> barycentric_coordinates(double x, double y, uint32_t i, uint32_t j, uint32_t k);
};

Triangulation::Triangulation(Delaunator& D) {
//    delauany = &D;
    uint32_t n = D.coords.size() / 2;
    uint32_t m = D.triangles.size() / 3;
    std::vector<int64_t> points_shape = {n, 2};
    std::vector<int64_t> triangles_shape = {m, 3};
...
std::pair<int32_t, parlay::sequence<double>> Triangulation::check_adj_triangles(double x, double y, uint32_t t) {
...
std::pair<int32_t, parlay::sequence<double>> Triangulation::check_neighborhood(double x, double y, int32_t pi) {
...
parlay::sequence<double> Triangulation::barycentric_coordinates(double x, double y, uint32_t t) {
