...
public:
    ...
    const pyarr_size_t* ppoints;
    std::vector<size_t> triangles;
    std::vector<std::vector<size_t>> point2tri;
    std::unordered_map<uint64_t, std::array<size_t, 2>> edge2tri;
    
    Triangulator(const pyarr_size_t& points, size_t n_jobs = 1) : n_jobs_(n_jobs) {
        // py::scoped_interpreter guard{};
        // std::cout << pypoints.shape()[0] << ' ' << pypoints.shape()[1] << std::endl;
        ...
        for (size_t i = 0; i < n; ++i) {
            double_points.at(2 * i) = static_cast<double>(points.at(i, 0));
            double_points.at(2 * i + 1) = static_cast<double>(points.at(i, 1));
        }
        ...
        for (size_t i = 0; i < triangles.size() / 3; ++i) {
            ...
            for (size_t j = 0; j < 3; ++j) {
                auto a = triangles.at(k + j);
                auto b = triangles.at(k + fast_mod(j + 1, 3));
                point2tri.at(a).push_back(i);
    ...
    inline std::optional<std::pair<size_t, std::array<int64_t, 4>>> locate_point(size_t x, size_t y, size_t neighbor) const {
        size_t nx = ppoints -> at(neighbor, 0);
        size_t ny = ppoints -> at(neighbor, 1);
        int64_t curr_t = -1;
        ...
        for (size_t t : point2tri.at(neighbor)) {
            ...
            for (size_t j = 0; j < 3; ++j) {
                ...
                size_t ax = ppoints -> at(a, 0);
                size_t ay = ppoints -> at(a, 1);
                size_t bx = ppoints -> at(b, 0);
                size_t by = ppoints -> at(b, 1);
        ...
        while (true) {
            ...
            size_t t = (adj_t[0] == static_cast<size_t>(curr_t)) ? adj_t[1] : adj_t[0];
            auto coords_info = barycentric_coords(x, y, t);
            if (point_in_triangle(coords_info)) {
                return {std::make_pair(t, coords_info)};
            }
    ...
    inline std::array<int64_t, 4> barycentric_coords(size_t x, size_t y, size_t t) const {
        size_t r1 = triangles[t];
        auto x1 = static_cast<int64_t>(ppoints -> at(r1, 0));
        auto y1 = static_cast<int64_t>(ppoints -> at(r1, 1));
        size_t r2 = triangles[t + 1];
        auto x2 = static_cast<int64_t>(ppoints -> at(r2, 0));
        auto y2 = static_cast<int64_t>(ppoints -> at(r2, 1));
        size_t r3 = triangles[t + 2];
        auto x3 = static_cast<int64_t>(ppoints -> at(r3, 0));
        auto y3 = static_cast<int64_t>(ppoints -> at(r3, 1));
