...
public:
    ...
    Triangulator(const pyarr_size_t& pypoints, const pyarr_size_t& triangles, int n_jobs = 1); // for checking algorithms on another triangulations
...
Triangulator::Triangulator(const pyarr_size_t& pypoints, const pyarr_size_t& pytriangles, int n_jobs = 1) {
    if (n_jobs < 0 and n_jobs != -1) {
            throw std::invalid_argument(
                "Invalid number of workers, have to be -1 or positive integer");
        }
        ...
        for (size_t i = 0; i < m; ++i) {
            size_t j = 3 * i;
            triangles.at(j) = static_cast<size_t>(pytriangles.at(j, 0));
            triangles.at(j + 1) = static_cast<size_t>([pytriangles.at(j, 1));
            triangles.at(j + 2) = static_cast<size_t>([pytriangles.at(j, 2));
        }
